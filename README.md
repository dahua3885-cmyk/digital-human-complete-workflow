...
当前版本为 **0.9.8**。仓库提供流程路由、首次建档、授权、任务订单、严格哈希交接和四个可独立安装的通用阶段 Skill：二创、Chatterbox 中文数字人音频、MuseTalk 1.5 数字人画面、固定竖屏剪辑包装。朋友安装后不需要再寻找示例 Skill 或自行拼接执行器。首次使用声音或画面时，仍需经用户同意在其电脑下载数 GB 公开模型；用户自己的声音、肖像和授权材料永远不进入仓库。
...
四个阶段均已绑定随包阶段 Skill。第一次真正制作时会只检查本次需要的阶段；声音阶段按需安装固定版本 Chatterbox 与公开模型，画面阶段检查 NVIDIA GPU、Python 3.10、FFmpeg 和 MuseTalk 公开模型，包装阶段准备 Pillow、FFmpeg 与固定中文字体。已有环境直接复用，没有时先说明预计下载量和电脑要求，经用户同意后由 Codex 运行相应准备器。该检查发生在创建任务订单之前，不能制作到一半才暴露依赖。
...
安装包包含四个阶段的公开实现，但不会在安装时偷偷下载大型模型。实际发起任务后，Codex 会先运行 `preflight_runtime.py`，并只为本次需要的阶段准备环境：
...
只有 JSON 中 `ready` 为 `true` 时，才表示所选阶段能在当前电脑运行。若用户希望替换自己的声音或包装引擎，也可以修改私有 Profile，但默认安装无需另找阶段 Skill。详细步骤见[下载、安装与可运行验收](references/installation-and-runtime.md)。
