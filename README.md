...
当前版本为 **0.9.1-draft 公开预览版**。仓库提供流程路由、首次建档、数字人资产中心、任务订单、阶段确认、路径与哈希交接、授权与安全规则、安装和发行校验；不附带四个通用阶段执行器、模型权重、第三方服务、人物声音或肖像素材。因此，它是可安装的编排层，不是无需配置即可生成成片的一键模型包。
...
首次调用时，Skill 会把最新版客户文档复制到当前 Codex 活动工作区，再给出两个可点击文档。这样可避免部分 Codex 环境无法直接打开 `.codex/skills/` 安装目录文件的问题：
