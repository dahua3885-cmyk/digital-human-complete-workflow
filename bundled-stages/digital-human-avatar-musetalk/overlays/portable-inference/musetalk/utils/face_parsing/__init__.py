    ...
    def __call__(self, image, size=(512, 512), mode="raw"):
        ...
        with torch.no_grad():
            ...
            if mode == "neck":
                ...
                # Preserve the subject's original cheeks and jawline. Only
                # blend the generated lips / oral cavity with a small margin.
                mouth_region = np.isin(parsing, [11, 12, 13]).astype(np.uint8) * 255
                mouth_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (15, 11))
                parsing = cv2.dilate(mouth_region, mouth_kernel, iterations=1)
            elif mode == "jaw":
